...
/// create the item with encrypted metadata + a reference to the blob.
export async function uploadFile(input: {
  ...
  const wrappedItemKey = seal(itemKey, input.masterKey);
...
/// Decrypt the metadata JSON for an existing file item.
export function decryptFileMeta(item: Item, masterKey: Uint8Array): FileMeta {
  if (!item.encrypted_body || !item.wrapped_item_key) {
    return { filename: item.title || 'file', mime: 'application/octet-stream', size: 0 };
  }
  const itemKey = openSeal(fromBase64(item.wrapped_item_key), masterKey);
